        ...
        for idx, (x_s, y_s) in enumerate(source_locations):
            ...
            for x_g, y_g in receivers_locations:
                ...
                if args.phase_only:
                    ...
                    h_n = np.exp(1j * 2 * np.pi * (r_g) / lambda_0) * np.exp(-1j * 2 * np.pi * (r_s) / lambda_0)
                h += h_n
...
if __name__ == "__main__":
    ...
    parser.add_argument('--phase_only', action='store_true', help='by setting the magnitude of the data to a constant')
    parser.add_argument('--full_phase_only', action='store_true', help='phase-only integration kernel')
