...
def calculate(source_scatter):
    ...
    for n in range(64):
        ...
        for x_cc, y_cc in source_scatter:
            for idx, (x_s, y_s) in tqdm(enumerate(source_locations)):
                ...
                for x_g, y_g in (receivers_locations):
                    ...
                    h_n = np.exp(1j * 2 * np.pi * (r_g) / lambda_n) * np.exp(-1j * 2 * np.pi * (r_s) / lambda_n)
                    h += h_n
                h_whole[y_idx, x_idx] = h
            temp += h_whole
        np.save(f'save_path_{n}.npy', temp)
...
if __name__ == "__main__":
    ...
    np.save('save_path', complex_wave)